...
-export([
    token_for_app/1, token_for_app/2,
...
-spec token_for_user(kz_term:ne_binary(), kz_term:ne_binary(), kz_term:ne_binary()) ->
    {'ok' | 'error', map()}.
...
add_provider_claims(
    #{
        auth_provider := #{jwt_flow := #{claims := ProviderClaims}},
        claims := Claims
    } = Map
) ->
    Map#{claims => maps:merge(Claims, add_claims(ProviderClaims))};
add_provider_claims(#{} = Map) ->
    Map.

add_app_claims(
    #{
        auth_app := #{jwt_flow := #{claims := AppClaims}},
        claims := Claims
    } = Map
) ->
...
add_optional_claims(
    #{
        options := #{claims := OptionalClaims},
        claims := Claims
    } = Map
) ->
    Map#{claims => maps:merge(Claims, OptionalClaims)};
add_optional_claims(#{} = Map) ->
    Map.
...
add_audience(
    #{
        auth_app := #{jwt_flow := #{audience := Aud}},
        claims := Claims
    } = Map
) ->
    Map#{claims => Claims#{aud => Aud}};
add_audience(
    #{
        auth_provider := #{jwt_flow := #{audience := Aud}},
        claims := Claims
    } = Map
) ->
    Map#{claims => Claims#{aud => Aud}};
...
add_subject(#{options := #{subject := SubjectId}} = Map) ->
...
add_subject(#{options := #{auth_id := AuthId}} = Map) ->
...
add_subject_claim(
    ...
    case kz_maps:get(Sub, Profile) of
        'undefined' ->
            case kz_maps:get(kz_term:to_atom(Sub, 'true'), Subject) of
                'undefined' -> Map;
                Value -> Map#{claims => Claims#{sub => Value}}
            end;
        Value ->
            Map#{claims => Claims#{sub => Value}}
    end;
add_subject_claim(
    #{
        auth_provider := #{jwt_flow := #{sub := Sub}},
        subject := #{profile := Profile} = Subject,
        claims := Claims
    } = Map
) ->
...
jwt_request(
    ...
    Body = string:join(
        lists:append(lists:map(fun({K, V}) -> [string:join([K, V], "=")] end, Fields)), "&"
    ),
        ...
        {_Reply, _Code, _RespHeaders, RespBody} ->
            ...
            Map#{
                error => kz_term:to_binary(
                    io_lib:format("unable to request service token: ~p", [RespBody])
                )
            }
    end.
...
refresh_token_flow(
    ...
    Fields = [
        {"client_id", kz_term:to_list(AppId)},
...
cache_token(#{from_cache := 'true'} = Map) ->
    Map;
cache_token(
    #{
        subject := #{'_id' := Id},
